...
( function( $, Mpress ) {
	'use strict';
	var $body, $elements, counter = 0;

	var init = function() {
		// Defer until document ready
	...
	function Toggle( $el ) {
		...
		$el.toggle = function( action ) {
			var aria, completed = false;
			// Set aria to either true or false, depending on action
			aria = ( action === 'activate' );
			// Trigger start event for other functions to bind to
			$el.trigger( 'toggle:start' );
			// Perform action
			$el.off( options.transitionEnd ).finish().attr( 'aria-expanded', aria ).removeClass( antistates[action].transition ).addClass( states[action].transition ).animateToggle( action ).one( options.transitionEnd, { action : action }, function( event ) {
				completed = true;
		...
		( function() {
			...
			$el.children().on( options.transitionEnd, function( event ) {
				event.stopPropagation();
			});
		})();
	...
	function Button( $el ) {
		...
		var cacheDom = function() {
			...
			$toggles = $.map( $( '[data-toggle=' + name + ']' ).not( $( '.menu-toggle' ) ), function( el ) {
		...
		var handleCompletion = function( event, data ) {
		 ...
		 var toggleState = function( event ) {
			 ...
			 for( var j = 0; j < $toggles.length; j++ ){
				 $toggles[j].one( 'toggle:done', handleCompletion );
				 // $toggles[j].trigger( 'toggle:mpress', [{ action : action }] );
				 $toggles[j].toggle( action );
			 }
			 return;
		 };
		 /**
